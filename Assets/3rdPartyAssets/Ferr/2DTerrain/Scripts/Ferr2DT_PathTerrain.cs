...
public partial class Ferr2DT_PathTerrain : MonoBehaviour, Ferr2D_IPath, IBlendPaintable {
    ...
    [SerializeField] Ferr2DT_TerrainMaterial terrainMaterial = null;
	[SerializeField] UnityEngine.Object      terrainMaterialInterface = null;
	...
	private Mesh   GetMesh() {
		...
		if (IsPrefab()) {
			...
			if (filter.sharedMesh == null || filter.sharedMesh.name != newName) {
				...
				string assetPath = UnityEditor.PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(this);
				#else
				string assetPath = UnityEditor.AssetDatabase.GetAssetPath(this);
				#endif
				string meshFolder = System.IO.Path.GetDirectoryName(assetPath) + "/Meshes/";
...
#if UNITY_EDITOR
		#if UNITY_2018_3_OR_NEWER
		isPrefab = UnityEditor.PrefabUtility.GetPrefabAssetType(gameObject) != UnityEditor.PrefabAssetType.NotAPrefab && !UnityEditor.PrefabUtility.IsPartOfPrefabInstance(gameObject);
		#else
	...
	void CreateCollider3D() {
		...
		List<PhysicsMaterial> materials = new List<PhysicsMaterial>();
        List<List<Vector2>>  segs      = GetColliderVerts(null, materials);

		ComponentPool<MeshCollider, PhysicsMaterial> pool = new ComponentPool<MeshCollider, PhysicsMaterial>(gameObject, materials, false);
	...
	public List<List<Vector2>>  GetColliderVerts  (List<PhysicsMaterial2D> aPhysicsMaterials2D=null, List<PhysicsMaterial> aPhysicsMaterials3D=null) {
		if (IsLegacy)
