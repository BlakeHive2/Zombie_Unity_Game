...
public partial class Ferr2DT_PathTerrainEditor : Editor {
	...
	private void  ShowPointControls   (Ferr2DPath aPath, int i, PathEditorUtil.Selection aSelection, Matrix4x4 aTransform, SerializedProperty aPathProp) {
		...
		if (terrain.EdgeMode != Ferr2D_SectionMode.None && (i == activePoint || aSelection.IsSelected(i) || pointData.scale != 1)) {
			...
			if (Event.current.alt) {
				...
				var fmh_288_53_638622707930907178 = Quaternion.identity; Vector3 move = Handles.FreeMoveHandle(scalePos, pointSize*sizeSmallHandle, Vector3.zero, Ferr2DT_Caps.GetScaleCap(screenNormal));
				if (move != scalePos) {
	...
	public override void OnInspectorGUI() {
		...
		// Check if the mesh has changed since we were last looking, this
		// most often will mean that the prefab has been reverted.
		if (lastMesh != terrain.MeshData) {
			lastMesh        = terrain.MeshData;
			cachedColliders = null;
